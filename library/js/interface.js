...
function changer_interface(idsSrc,idsDst,elem){
		...
		if (test_model=="true" && test_save==0){
			//alert(elem.id);
			var sql = 'INSERT INTO svg(fichier,isModel,titre,id_user) VALUES(?1,?2,?3,?4);';
			var statement = mDBConn.createStatement(sql);
		...
		if (test_model2=="true"){
			...
			var sql = 'UPDATE svg SET fichier= ?1 WHERE id_svg=?2;';
			//alert("changer_interface(\'"+idsDst+"\',\'"+id_courant+"\',this)"+"  <---  changer_interface(\'"+id_interface2+"\',\'"+id_interface+"\',this)");
			//alert(xml3);
			var statement = mDBConn.createStatement(sql);
