...
function AjoutOntoActeur(){
	...
	cells = new Array("_fic_"+numFic,file.leafName,'','Fichier � traiter')
...
function createDefault(){
	...
	if (myArray1.length<2)	{
		...
		statement.bindUTF8StringParameter(4,"false");
		statement.execute();
		statement.reset();
...
function changer_interface(idsSrc,idsDst,elem){
		...
		if (test_model=="true"){
			...
			statement.execute();
			statement.reset();
			if (id_courant!=idsDst && isAdded(id_interface)=="false"){
			//alert("!=");
			...
			else if (id_courant==idsDst && isAdded(id_interface)=="false"){
				//alert('==');
			...
			if (id_courant!=idsDst && isAdded(id_courant)=="false"){
			//alert("!=");
		    	var pop = document.getElementById(racine_interface); // a <menupopup> element
				var first = createMenu(titre+"s");
				var pop2=createMenuPopup("pop_interface_"+id_courant);
				var last = createMenuItem(titre);
				last.setAttribute("oncommand","affiche_interface('fig_p','','fig_p','"+id_courant+"');");
				last.setAttribute("id","item_"+id_interface);
				pop2.appendChild(last);
				first.appendChild(pop2);
				pop.appendChild(first);
			...
			else if (id_courant==idsDst && isAdded(id_courant)=="false"){
				//alert('==');
				var pop = document.getElementById("test3"); // a <menupopup> element
				var first = createMenu(titre+"s");
				var pop2=createMenuPopup("pop_interface_"+id_courant);
		...
		if (test_model2=="true"){
			...
			if (id_courant!=idsDst && isAdded(id_interface2)=="false"){
			//alert("!=");
		    	var pop = document.getElementById(racine_interface); // a <menupopup> element
			...
			else if (id_courant==idsDst && isAdded(id_interface2)=="false"){
				//alert('==');
				var pop = document.getElementById("test3"); // a <menupopup> element
				var first = createMenu(titre2+"s");
				var pop2=createMenuPopup("pop_interface_"+idsDst);
				var last = createMenuItem(titre);
				last.setAttribute("oncommand","affiche_interface('fig_p','','fig_p','"+idsDst+"');");
				last.setAttribute("id","item_"+idsDst);
				pop2.appendChild(last);
				first.appendChild(pop2);
				pop.appendChild(first);
				racine_interface="pop_interface_"+idsDst;
		...
		//alert (test_model+" isModified : "+isModified);
		if (test_model=="true" && isModified=="false"){
			...
			statement.execute();
			statement.reset();
			//alert("changer_interface(\'"+idsSrc+"\',\'"+idsDst+"\',this)","changer_interface(\'"+id_interface+"\',\'"+id_interface2+"\',this)");
			...
			var sql = 'UPDATE xul SET form_xul= ?1 WHERE id_svg=?2;';
			//alert('UPDATE svg SET fichier= '+'hello'+' WHERE id_svg='+id_interface+';');
			//alert("False ::: changer_interface(\'"+idsSrc+"\',\'"+idsDst+"\',this)"+"  <---  changer_interface(\'"+id_interface+"\',\'"+id_interface2+"\',this)");
			var statement = mDBConn.createStatement(sql);
			statement.bindUTF8StringParameter(0,xml4);
			statement.bindUTF8StringParameter(1,id_interface);
			statement.execute();
...
function isAdded (id_svg){
	...
	//alert("item_"+id_svg+" : "+test_svg);
	for (i=1;i<list_svg.length;i++)
	{
		alert(list_svg[i].getAttribute("id")+" : "+"item_"+id_svg);
...
function Save_SVG(){
		...
		var mDBConn = connect_DB();
		var sql = 'INSERT INTO svg(fichier,isModel,titre,isModified,id_user) VALUES(?1,?2,?3,?2,?4);';
		var statement = mDBConn.createStatement(sql);
...
function save_current_svg(){
	...
	if (test_model=="true"){
	    ...
	    statement.bindUTF8StringParameter(0,svg_src);
	    statement.bindUTF8StringParameter(1,id_m);
	    statement.bindUTF8StringParameter(2,titre+" - "+user);
	    statement.bindUTF8StringParameter(3,id_user);
	    statement.bindUTF8StringParameter(4,"false");
	    statement.execute();
    	statement.reset();
