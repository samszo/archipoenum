...
function AjoutOntoActeur(){

	GetTreeValSelect(treesite,0);
	cells = new Array("_fic_"+numFic,file.leafName,'','Fichier à traiter')
...
function SelectSvgUser(id_user){
	...
	return boucle_select(statement);
}

function createDefault(){
	var mDBConn = connect_DB();
	myArray1=SelectSvgUser(0);
	if (myArray1.length<2)	{
		...
		insertIntoSvg(xml1,"true","Acteur","0","false");
...
function insertIntoSvg(fichier,isModel,titre,isModified,id_user){
	var mDBConn = connect_DB();
	var sql = 'INSERT INTO svg(fichier,isModel,titre,isModified,id_user) VALUES(?1,?2,?3,?5,?4);';
	var statement = mDBConn.createStatement(sql);
	statement.bindUTF8StringParameter(0,fichier);
	statement.bindUTF8StringParameter(1,isModel);
	statement.bindUTF8StringParameter(2,titre);
	statement.bindUTF8StringParameter(3,isModified);
	statement.bindUTF8StringParameter(4,id_user);
	statement.execute();
	statement.reset();
}

function insertIntoXul(id_element,form_xul,id_svg){
	var mDBConn = connect_DB();
	var sql = 'INSERT INTO xul(id_element,form_xul,id_svg) VALUES(?1,?2,?3);';
	var statement = mDBConn.createStatement(sql);
	statement.bindUTF8StringParameter(0,id_element);
	statement.bindUTF8StringParameter(1,form_xul);
...
function Open_default(elem){
	...
	var metadonnees = createMetaData();
	var bag=createRdfBag();
	var creator=createRdfCreator();
	var li=parser.parseFromString('<rdf:li xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">BARGAOUI Abderrahim</rdf:li>',"text/xml");
	var RDF=createRDF();
	//var rdfstr='<rdf:Description about="http://exemple.org/montruc" dc:title="Rapport financier sur montruc" dc:description="$three $bar $thousands $dollars $from 1998 $through 2000" dc:publisher="Organisation Exemple" dc:date="2000-04-11" dc:format="image/svg+xml" dc:language="fr" ><dc:creator><rdf:Bag><rdf:li>Irving BIRD</rdf:li><rdf:li>Mary LAMBERT</rdf:li></rdf:Bag></dc:creator></rdf:Description>';
	//alert(RDF);
	var rdfdesc=createRdfDesc("Test1","Abderrahim");
	//alert(rdfdesc);
	bag.appendChild(li.documentElement);
	creator.appendChild(bag);
	rdfdesc.appendChild(creator);
	RDF.appendChild(rdfdesc);
	metadonnees.appendChild(RDF);
	resultDoc.documentElement.appendChild(metadonnees);
	...
	var statement = mDBConn.createStatement('SELECT * FROM xul where id_svg=?1;');
	statement.bindUTF8StringParameter(0,"1");
...
// Changer le couleur de graphe séléctionné aprés la validation des donn�es saisies
...
//	try {
		...
		for (var i = 0; i < arrId.length; i++){
			...
			if (arrId[i]=="fig_18_ontoactor")
				document.getElementById("fig_18_actor").setAttribute("fill","green");
		}
...
function changer_interface(idsSrc,idsDst,elem){
		...
		var statement = mDBConn.createStatement('SELECT id_svg,titre,isModel,isModified FROM svg where id_svg=?1;');
		statement.bindUTF8StringParameter(0,id_courant);
		var myArray1 = boucle_select(statement);
		var id_m=myArray1[j]['id_svg'];
		var test_model =myArray1[j]['isModel'];		
		var titre=myArray1[j]['titre'];
		var isModified=myArray1[j]['isModified'];
		...
		if (test_model=="true"){
			insertIntoSvg(svg_src,id_m,titre+" - "+user,id_user,"false");		    	
			...
			insertIntoXul("saisie_"+id_interface,xul_src,id_interface);

		
		}
		else {
			id_interface =id_courant;
		...
		if (test_model2=="true"){
			insertIntoSvg(svg_src2,id_m,titre2+" - "+user,id_user,"false");
	    	
	    	var statement = mDBConn.createStatement('SELECT id_svg FROM svg ORDER BY id_svg DESC;');
			var myArray1 = boucle_select(statement);
			id_interface2 =myArray1[j]['id_svg'];
			insertIntoXul("saisie_"+id_interface2,form_xulDst,id_interface2);
			form_xul =myArray1[j]['form_xul'];

		}
		else {
			id_interface2 =idsDst;

		}
		if (test_model=="true" && isModified=="false"){
			xml2=RC(interface_modif,"changer_interface(\'"+idsSrc+"\',\'"+idsDst+"\',this)","changer_interface(\'"+id_interface+"\',\'"+id_interface2+"\',this)");
			var sql = 'UPDATE svg SET fichier= ?1 , isModified=?3 WHERE id_svg=?2;';
			var statement = mDBConn.createStatement(sql);
			statement.bindUTF8StringParameter(0,xml2);
			statement.bindUTF8StringParameter(1,id_interface);
			statement.bindUTF8StringParameter(2,"true");
			statement.execute();
			statement.reset();
			...
			xml2=RC(svg_src,"changer_interface(\'"+idsSrc+"\',\'"+idsDst+"\',this)","changer_interface(\'"+id_interface+"\',\'"+id_interface2+"\',this)");
			var sql = 'UPDATE svg SET fichier= ?1 , isModified=?3 WHERE id_svg=?2;';
		...
		while (doc.hasChildNodes())	
			doc.removeChild(doc.firstChild);
		var parser=new DOMParser();
			if (idsSrc!=idsDst && isAdded(id_interface)=="false" && test_model!="true" && idsSrc!=""){
				...
				last.setAttribute("id","item_"+id_interface);
				pop2.appendChild(last);
				first.appendChild(pop2);
				pop.appendChild(first);
				racine_interface="pop_interface_"+id_courant;
				id_courant=id_interface;
			}
			else if (idsSrc==idsDst && isAdded(id_interface)=="false" && test_model!="true" && idsSrc!=""){
				var pop = document.getElementById(racine_interface); // a <menupopup> element
				var first = createMenu(titre+"s");
				var pop2=createMenuPopup("pop_interface_"+id_interface);
				var last = createMenuItem(titre);
			...
			if (idsSrc!=idsDst && isAdded(id_interface2)=="false" && test_model2!="true" && idsSrc!=""){
		    	var pop = document.getElementById(racine_interface); // a <menupopup> element
				var first = createMenu(titre2+"s");
				var pop2=createMenuPopup("pop_interface_"+id_interface2);
				var last = createMenuItem(titre2);
				last.setAttribute("oncommand","affiche_interface('fig_p','','fig_p','"+id_interface2+"');");
				last.setAttribute("id","item_"+id_interface2);
				pop2.appendChild(last);
				first.appendChild(pop2);
				pop.appendChild(first);
			...
			else if (idsSrc==idsDst && isAdded(id_interface2)=="false" && test_model2!="true" && idsSrc!=""){
				var pop = document.getElementById(racine_interface); // a <menupopup> element
				var first = createMenu(titre2+"s");
				var pop2=createMenuPopup("pop_interface_"+id_interface2);
				var last = createMenuItem(titre2);
		...
		resultDoc3=add_save(resultDoc2);
		p_saisi.appendChild(resultDoc3.documentElement);
		id_courant=id_interface2;
		g_idSvg=id_interface2;
		statement.reset();
		get_list_SVG_user();
	//} catch(ex2){alert("interface:changer_interface:"+ex2);}
}

function isAdded (id_svg){
	pop_list_svg=document.getElementById("test3");
	list_svg=pop_list_svg.getElementsByTagName("menuitem");
	test_svg="false";
	var l=list_svg.length;
	for (i=1;i<list_svg.length;i++)
	{
		
		if (list_svg[i].getAttribute("id")=="item_"+id_svg){
			test_svg="true";
			break;
		}
		else {
	...
	try{
		...
		if (res == nsIFilePicker.returnOK){
			...
			if (cmp=='p')
			{
				...
				// Intégrer le DOM récupéré � l'interieur de document
...
function svg_open_id(id_svg)
{
	...
	if (isAdded(id_svg)=="false"){
    	var pop = document.getElementById(racine_interface); // a <menupopup> element
		var first = createMenu(titre+"s");
		var pop2=createMenuPopup("pop_interface_"+id_svg);
		var last = createMenuItem(titre);
		last.setAttribute("onclick","affiche_interface('fig_p','','fig_p','"+id_svg+"');");
		last.setAttribute("id","item_"+id_svg);
		pop2.appendChild(last);
		first.appendChild(pop2);
		pop.appendChild(first);
		racine_interface="pop_interface_"+id_svg;
		id_courant=id_svg;
	}
	while (p_saisi.hasChildNodes())	
		p_saisi.removeChild(p_saisi.firstChild);
...
try{
		...
		if (document.getElementById("p"+docs)==null){
			var first = createMenu("Liste des SVG");
			first.setAttribute("id","f"+docs);
			var pop = createMenuPopup("p"+docs);
		}
		else{
			var first =document.getElementById("f"+docs)
			var pop =document.getElementById("p"+docs)
...
function createDB(){
	try {
		...
		var myCreateDBQuery3 = 'CREATE TABLE IF NOT EXISTS tag_cloud (id_user uniqueidentifier , id_tag uniqueidentifier , id_hist uniqueidentifier,id_svg uniqueidentifier);';
		...
		var myCreateDBQuery9 = 'CREATE TABLE IF NOT EXISTS svg (id_svg INTEGER PRIMARY KEY AUTOINCREMENT,  titre text(40), fichier text(1000),isModel varchar(40),isModified varchar(40),id_user uniqueidentifier);';
...
function Save_SVG(){
		...
		var mDBConn = connect_DB();
		insertIntoSvg(svg,"false",nomFic,id_user,"false");
    	j=0;
	    var statement = mDBConn.createStatement('SELECT id_svg FROM svg ORDER BY id_svg DESC;');
		var myArray1 = boucle_select(statement);
		id_interface =myArray1[j]['id_svg'];		
...
function AppendSVG(url,doc) {
  try {
	...
	if (p.status != "200" ){
		...
		// Intégrer le DOM récupér à l'interieur de document
		doc.appendChild(resultDoc.documentElement);
...
function createRdfDesc(titre,publisher) {
  const XUL_NS ="http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "rdf:Description"); 
  item.setAttribute("dc:title",titre);
  item.setAttribute("dc:description", "archipoenum indexing");
  item.setAttribute("dc:publisher", publisher);
  item.setAttribute("dc:date", ladate.toGMTString());
  item.setAttribute("dc:format", "numérique");
  item.setAttribute("dc:language", "fr");
  return item;
}

function createRDF() {
  const XUL_NS = "http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "rdf:RDF"); // create a new XUL menuitem
  item.setAttribute("xmlns:rdf", "http://www.w3.org/1999/02/22-rdf-syntax-ns#");
  item.setAttribute("xmlns:rdfs", "http://www.w3.org/2000/01/rdf-schema#");
...
function createMetaData() {
  const XUL_NS = "http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "metadata");
  return item;
}

function createRdfCreator() {
  const XUL_NS = "http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "dc:creator");
  return item;
}

function createRdfBag() {
  const XUL_NS = "http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "rdf:Bag");
  return item;
}

function createRdfli(data) {
  const XUL_NS = "http://www.w3.org/1999/02/22-rdf-syntax-ns#";
  var item = document.createElementNS(XUL_NS, "rdf:li");
  item.insertData(0,data);
  return item;
}
...
{
	//récupération du numèro de graphique
	var c1 = c.split("_")[2];
	first= createVbox(c);
	g=createG("root");
...
function Valider_form(id_form)
{
	...
	if(g_idSvg==-1){    
		insertIntoSvg(xml,"true",svgTitre+" - "+user+" - model:"+ladate.toGMTString(),"0","false");	
		var statement = mDBConn.createStatement('SELECT id_svg FROM svg ORDER BY id_svg DESC;');
		j=0;
		var myArray1 = boucle_select(statement);
	...
	if (fct=="afficher_form"){
		...
		insertIntoXul(id_form,xul_complet,g_idSvg);
...
function add_save(elem){
	bts=elem.getElementsByTagName("button");
	for (i=0;i<bts.length;i++)
	{
		ancien_fct=bts[i].getAttribute("onclick");
		bts[i].setAttribute("onclick","save_current_svg();"+ancien_fct);
	}
	return(elem);
}

function save_current_svg(){
	netscape.security.PrivilegeManager.enablePrivilege('UniversalXPConnect');
	var mDBConn = connect_DB();
	var p_interface = document.getElementById("C1");
	doc=document.getElementById("fig_p");
